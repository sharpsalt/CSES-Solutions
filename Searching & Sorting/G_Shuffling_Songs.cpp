...
void dfs(int xx,vector<vector<int>>&adj,vector<bool>&visited,int cnt, map<pair<int,int>,int>&mppp){
    visited[xx]=true;
    for(auto v:adj[xx]){
        if(!visited[xx]){
            //matlab jispe nhi gye hai uspe jaeynge
            cnt+=mppp[{xx,v}];
        }
    }
}
...
void solve()
{
  ...
  if(n==1){
    ...
    //ab basically implementation hi hai ye bahut

    //pehle to hum isko compress krdenge
    map<string,int>mpp;
    for(int i=0;i<n;i++)
    {
        mpp[v[i].first]=1;
    ...
    //ab dono push krdiye ab graph banadenge
    cout<<graph<<endl;
    vector<vector<int>>adj(xx+1);
    for(int i=0;i<n;i++)
    {
        int a=graph[i].first;
        ...
        adj[a].pb(b);
        adj[b].pb(a);
        //dono me hum push krdiye hai a,b ko so ab
    }
    //ab bas dfs chalana hai jo maximum non connected components hai usko pure xx me se - krna pdega to get ans
    vector<bool>visited(xx+1,false);
    int maxi=INT_MIN;
    for(int i=1;i<=xx;i++){
        int cnt=0;
        if(!visited[i]){
            dfs(i,adj,visited,cnt,mppp);
        }
        maxi=max(maxi,cnt);
    }
  }
