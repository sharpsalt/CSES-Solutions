...
// GP_Hash_Table-gp_hash , Segment Tree with Point(Adv)-SegPoint Adv, Segment Tree with Point(Basic)-SegPoint Basic , Mathematics-Maths
...
void solve()
{
  ...
  queue<int>q;
  vector<int>pp(n+1,INT_MAX);
  q.push(0);//source vertex ghusa diye
  bool mila=false;
  while(!q.empty()){
    int v=q.front();
    q.pop();
